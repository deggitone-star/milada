...
interface CollageGalleryProps {
  ...
  /** Индексы фото, которые делаем крупными (2×2). По умолчанию — ровная сетка. */
  featured?: number[];
}

export default function CollageGallery({ images, featured = [] }: CollageGalleryProps) {
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null);

  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 auto-rows-[170px] lg:auto-rows-[210px] gap-3 lg:gap-4">
        {images.map((img, i) => {
